    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-6">
        ...
        <div className="text-right space-y-1 shrink-0">
          <div className={cn("text-3xl font-bold", getScoreColor(agent.score))}>{getScoreBadge(agent.score)} {agent.score}</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { label: "Price", value: `$${agent.price.toFixed(4)}` },
          { label: "24h Change", value: `${isPositive ? "+" : ""}${agent.priceChange24h.toFixed(1)}%`, color: isPositive ? "text-green-400" : "text-red-400", Icon: isPositive ? TrendingUp : TrendingDown },
          ...
          <Card key={m.label} className="bg-zinc-900 border-zinc-800">
            <CardContent className="p-4">
              <p className="text-xs text-zinc-500 flex items-center gap-1">{m.Icon && <m.Icon className="h-3 w-3" />}{m.label}</p>
              <p className={cn("text-lg font-bold text-white", m.color)}>{m.value}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {scoreItems.map((item) => (
              <div key={item.label} className="space-y-1">
                ...
                <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-green-600 to-green-400 rounded-full" style={{ width: `${item.value}%` }} />
                </div>
              </div>
            ))}
          </CardContent>
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader><CardTitle className="text-white">AI Analysis</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <p className="text-zinc-300 text-sm">{agent.summary}</p>
            <Separator className="bg-zinc-800" />
            <div>
              <h4 className="text-sm font-semibold text-green-400 mb-2">✅ Strengths</h4>
              <ul className="space-y-1">{agent.strengths.map((s, i) => <li key={i} className="text-sm text-zinc-400 flex items-start gap-2"><span className="text-green-400 mt-0.5">•</span> {s}</li>)}</ul>
            </div>
            <div>
              <h4 className="text-sm font-semibold text-red-400 mb-2">⚠️ Weaknesses</h4>
              <ul className="space-y-1">{agent.weaknesses.map((w, i) => <li key={i} className="text-sm text-zinc-400 flex items-start gap-2"><span className="text-red-400 mt-0.5">•</span> {w}</li>)}</ul>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader><CardTitle className="text-white">Recent Trades</CardTitle></CardHeader>
        <CardContent>
              ...
              <TableBody>
                {agent.trades.slice(0, 10).map((trade) => (
                  <TableRow key={trade.id} className="border-zinc-800">
                    <TableCell>
                      <Badge variant="outline" className={cn("text-xs", trade.type === "buy" ? "text-green-400 border-green-500/20 bg-green-500/10" : "text-red-400 border-red-500/20 bg-red-500/10")}>
                        {trade.type.toUpperCase()}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-white font-medium">{trade.amount.toLocaleString()}</TableCell>
                    <TableCell className="text-zinc-300 hidden sm:table-cell">${trade.price.toFixed(4)}</TableCell>
                    <TableCell className="text-zinc-500 font-mono text-xs hidden md:table-cell">{trade.trader}</TableCell>
                    <TableCell className="text-zinc-500 text-right text-sm">{timeAgo(trade.timestamp)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
